...
def RecentFileAccess(low_timeline, start_id, end_id):
    ...
    for each_event in trigger_matches:
        ...
        reasoning.description = f"Recent file access {','.join(each_event.provenance['raw_entry'])}"
        reasoning.test_event = test_event_dict
...
def GetMatchedFileDetailsFromTrigger(event):
...
def GetMatchedFileDetailsFromLNK(event):
    """Extracts file details from the trigger event"""
    file_details_dict = {}
...
def GetMatchedFileDetailsFromEntryShell(event):
