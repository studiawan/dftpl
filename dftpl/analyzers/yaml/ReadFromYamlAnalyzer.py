...
def CreateHighTimeline(low_timeline: LowLevelTimeline, yaml_rule, start_id: int=0, end_id: int=None) -> HighLevelTimeline:
    ...
    test_event.type = yaml_rule['test_event']['type']
    test_event.evidence = yaml_rule['test_event']['evidence']
    ...
    for each_low_event in trigger_matches:
        ...
        high_event.description = yaml_rule['high_level_event']['description']
        ...
        reasoning.description = yaml_rule['reasoning']['description'] + ' ' + eval(yaml_rule['reasoning']['found_in'])
